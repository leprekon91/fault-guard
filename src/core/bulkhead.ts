...
class Pool {
    ...
    constructor(private limit: number, private queueLimit: number, private monitor?: Monitor, private key?: PoolKey) {}
    ...
    isIdle(idleTimeoutMs: number) {
        return Date.now() - this.lastUsed > idleTimeoutMs && this.current === 0 && this.queue.length === 0;
    }
    ...
    private next() {
        ...
        if (!job) return;
        this.monitor?.({ type: 'bulkhead.dequeue', payload: { key: this.key, queueLength: this.queue.length } });
        job();
    }
    ...
    public async exec<T>(fn: () => Promise<T>): Promise<T> {
        ...
        if (this.current < this.limit) {
            this.current++;
            this.monitor?.({ type: 'bulkhead.acquire', payload: { key: this.key, current: this.current } });
            try {
                return await fn();
            } finally {
                this.current--;
                this.touch();
                this.monitor?.({ type: 'bulkhead.release', payload: { key: this.key, current: this.current } });
                this.next();
            }
        }
        ...
        if (this.queue.length < this.queueLimit) {
            return new Promise<T>((resolve, reject) => {
                ...
                this.touch();
                this.monitor?.({ type: 'bulkhead.queued', payload: { key: this.key, queueLength: this.queue.length } });
            });
        }

        this.monitor?.({ type: 'bulkhead.reject', payload: { key: this.key, queueLength: this.queue.length } });
...
export class Bulkhead {
    ...
    constructor(
        ...
        private opts?: { keyed?: boolean; idleTimeoutMs?: number; maxKeys?: number; keyFn?: () => string },
    ) {
    ...
    public async exec<T>(keyOrFn: string | (() => Promise<T>), maybeFn?: () => Promise<T>): Promise<T> {
